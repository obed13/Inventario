...
$consulta = "SELECT p.product_name,p.note,p.status,p.buying_price,m.name as empleado,u.name as ubicacion FROM products p,manufacturers m,ubicacion u WHERE p.manufacturer_id=m.id AND p.ubicaciones_id = u.id AND u.name LIKE '%".$query."%' ORDER BY u.name ASC";
...
->setCellValue('A1','Ubicacion')
    ...
    while ($fila = $resultado->fetch_array()) {
        ...
        $spreadsheet->getActiveSheet(0)
            ...
            ->setCellValue('A'.$i, $fila['ubicacion'])
    ...
    header('Content-Disposition: attachment; filename="Reporte_Ubicacion.xlsx"');
