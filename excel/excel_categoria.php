...
$consulta = "SELECT p.product_name,p.note,p.status,p.buying_price,m.name as empleado,c.name as categoria,u.name as ubicacion FROM products p,manufacturers m,categoria c,ubicacion u WHERE p.manufacturer_id=m.id AND p.categoria_id=c.id AND p.ubicaciones_id = u.id AND c.name LIKE '%".$query."%' ORDER BY c.name ASC";
