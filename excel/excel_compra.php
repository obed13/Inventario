...
$consulta = "SELECT p.product_name,p.note,p.status,p.buying_price,m.name as empleado,u.name as ubicacion,.p.presentation FROM products p,manufacturers m,ubicacion u WHERE p.manufacturer_id=m.id AND p.ubicaciones_id = u.id AND p.presentation LIKE '%".$query."%' ORDER BY p.presentation ASC";
...
->setCellValue('A1','Orden Compra')
    ...
    while ($fila = $resultado->fetch_array()) {
        ...
        $spreadsheet->getActiveSheet(0)
            ...
            ->setCellValue('A'.$i, $fila['presentation'] )
