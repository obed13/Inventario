  ...
  <body class="hold-transition <?php echo $skin;?> sidebar-mini">
	<?php 
		if ($permisos_agregar==1){
			include("modal/agregar_ubicacion.php");
		}
	...
	function per_page(valor){
		...
		function eliminar(id){
			if(confirm('Esta acción  eliminará de forma permanente la ubicación \n\n Desea continuar?')){
				var page=1;
				var query=$("#q").val();
				var per_page=$("#per_page").val();
  ...
  if (e.isDefaultPrevented()) {
    ...
    $('#guardar_datos').attr("disabled", true);
	 var parametros = $(this).serialize();
		 $.ajax({
				type: "POST",
				url: "ajax/registro/agregar_ubicacion.php",
				data: parametros,
				 beforeSend: function(objeto){
...
$('#update_register').validator().on('submit', function (e) {
 ...
 var parametros = $(this).serialize();
	 $.ajax({
			type: "POST",
			url: "ajax/modificar/ubicacion.php",
			data: parametros,
			 beforeSend: function(objeto){
				$("#resultados_ajax").html("Enviando...");
			  },
...
<script>
		function editar(id){
			...
			$.ajax({
					url:'modal/editar/ubicacion.php',
					data: parametros,
					 beforeSend: function(objeto){
