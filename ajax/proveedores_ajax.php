<?php
	...
	$query_validate=mysqli_query($con,"select supplier_id from purchases where supplier_id='".$id."'");
	$count=mysqli_num_rows($query_validate);
		if ($count==0)
		{
			if($delete=mysqli_query($con, "DELETE FROM suppliers WHERE id='$id'") and $delete2=mysqli_query($con,"DELETE FROM contacts_supplier WHERE supplier_id='$id' ")){
				$aviso="Bien hecho!";
				...
				<div class="table-responsive">
					<table class="table table-condensed table-hover table-striped">
						...
						while($row = mysqli_fetch_array($query)){	
							...
							$fecha=$d."-".$m."-".$Y;
							$sql_contacto=mysqli_query($con,"select first_name, last_name, phone, email from contacts_supplier where supplier_id='$id'");
