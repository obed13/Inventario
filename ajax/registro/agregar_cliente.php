...
if (empty($_POST['bussines_name'])){
			...
			// escaping, additionally removing everything that could be (html/javascript-) code
				...
				$created_at=date("Y-m-d H:i:s");
                    ...
                    $sql = "INSERT INTO customers (created_at,name,address1,city,state,postal_code,	country_id, work_phone, website, tax_number) VALUES('$created_at','$name','$address1','$city','$state','$postal_code', '$country_id','$work_phone','$website','$tax_number');";
                    $query = mysqli_query($con,$sql);

                    // if has been added successfully
                    if ($query) {
                        $messages[] = "El cliente ha sido creado con éxito.";
						$last=mysqli_query($con,"select LAST_INSERT_ID(id) as last from customers order by id desc limit 0,1 ");
						$rw=mysqli_fetch_array($last);
						$client_id=$rw['last'];
						$query2=mysqli_query($con,"insert into contacts (client_id, first_name, last_name, 	email, phone) values ('$client_id','$first_name','$last_name','$email', '$phone')");
                    } else {
