<?php
	...
	$query_validate=mysqli_query($con,"select categoria_id from products where categoria_id='".$id."'");
	$count=mysqli_num_rows($query_validate);
	if ($count==0){
			if($delete=mysqli_query($con, "DELETE FROM categoria WHERE id='$id'")){
				$aviso="Bien hecho!";
		...
		{
			$aviso="Aviso!";
			$msj="Error al eliminar los datos. La categoria se encuentra vinculado con un producto";
			$classM="alert alert-error";
			$times="&times;";
		}	
				...
				<div class="table-responsive">
					<table class="table table-condensed table-hover table-striped">
						...
						while($row = mysqli_fetch_array($query)){	
							...
							$count=mysqli_query($con,"select count(*) AS num_prod from products where categoria_id='".$id."'" );
