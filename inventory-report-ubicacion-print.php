...
ob_start();
	...
	if($action == 'ajax'){
		...
		$campos="products.product_id, products.product_code, products.product_name, products.note, products.status, products.buying_price, manufacturers.name as empleado, categoria.name, ubicacion.name as ubicacion,programa.name as programa,subcuenta.name as subcuenta";
		...
		$sWhere.=" and ubicacion.name LIKE '%".$query."%'";
		$sWhere.=" order by ubicacion.name ASC";
	...
	require_once (dirname(__FILE__).'/pdf/documentos/html/inventory-report-ubicacion.php');
